...
const CyclingGame = () => {
  ...
  // generateCards moved to src/game/gameLogic.js
...
// chooseCardToPlay moved to src/game/gameLogic.js
          ...
          {gr.map(([name, rider]) => (
            <div key={name} className="mb-2">
              <button
                ...
                className="w-full text-left px-4 py-3 bg-blue-500 text-white rounded hover:bg-blue-600 font-bold"
              >
                {name} <span className="text-sm text-gray-100">({rider.team})</span> {rider.win_chance > 0 && <span className="float-right mr-2">{Math.round(rider.win_chance)}%</span>}
              </button>
            </div>
          ))}
        </div>
        ...
        {gameState === 'setup' && (
          <div className="fixed left-0 right-0 bottom-0 p-4 bg-white border-t shadow-lg">
            <div className="max-w-7xl mx-auto px-4">
              <button onClick={initializeGame} className="w-full bg-blue-600 text-white py-4 rounded-lg text-xl font-bold flex items-center justify-center gap-3">

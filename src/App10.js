...
const confirmMove = () => {
  ...
  for (const name of nonAttackers) {
    ...
    try {
      ...
      if (finishPos !== -1 && oldPosition < finishPos && newPos >= finishPos) {
        const fieldsToFinish = finishPos - oldPosition; // how many fields into the move
        const speedForFraction = groupSpeed > 0 ? groupSpeed : (fieldsToFinish || 1);
        const fraction = Math.max(0, Math.min(1, fieldsToFinish / speedForFraction));
  // Use 1-based round number: prel_time = round*60 + fraction*60 where
  // 'round' here is 1-based (round 1 -> 1:00 base). The state `round`
  // is zero-based at start, so add 1.
  const prelSeconds = ((round + 1) + fraction) * 60; // seconds
        updatedCards[name] = { ...updatedCards[name], prel_time: prelSeconds };
        addLog(`Prel time set for group ${rider.group}: ${convertToSeconds(prelSeconds)}`);
      }
    } catch (e) {}
  ...
  if (finishPos !== -1 && newPos > finishPos) newPos = finishPos;
    ...
    try {
      if (finishPos !== -1 && oldPosition < finishPos && newPos >= finishPos) {
        const fieldsToFinish = finishPos - oldPosition;
        const speedForFraction = (m.effectiveValue + (i===0 && extra ? extra : 0)) || (fieldsToFinish || 1);
        const fraction = Math.max(0, Math.min(1, fieldsToFinish / speedForFraction));
  // Use 1-based round number for attackers as well
  const prelSeconds = ((round + 1) + fraction) * 60;
        updatedCards[name] = { ...updatedCards[name], prel_time: prelSeconds };
        addLog(`Prel time set for attacker ${name} (group ${rider.group}): ${convertToSeconds(prelSeconds)}`);
      }
    } catch (e) {}
  ...
  if (currentGroup > 1) {
    ...
    setTimeout(() => {
      setCards(prevCards => {
        ...
        try {
          const detected = detectSprintGroups(updatedCards, track);
          if (detected && detected.length > 0) {
            // Assign prel_time to all riders in the detected sprint groups
            const finishPos = track.indexOf('F');
            let latestPtLocal = latestPrelTime || 0;
            for (const g of detected) {
              for (const [n, r] of Object.entries(updatedCards)) {
                if (r.group !== g) continue;
                // Only consider riders that are at/over the finish
                if (typeof r.position !== 'number' || r.position < finishPos) continue;
                try {
                  const oldPos = (typeof r.old_position === 'number') ? r.old_position : (r.position || 0);
                  const fieldsToFinish = Math.max(0, finishPos - oldPos);
                  const speedForFraction = (groupSpeed && groupSpeed > 0) ? groupSpeed : 1;
                  const fraction = Math.max(0, Math.min(1, fieldsToFinish / speedForFraction));
                  const prelSeconds = ((round + 1) + fraction) * 60;
                  updatedCards[n] = { ...r, prel_time: prelSeconds };
                  latestPtLocal = Math.max(latestPtLocal, prelSeconds);
                  addLog(`Assigned prel_time for ${n} (group ${g}): ${convertToSeconds(prelSeconds)} (fraction=${fraction.toFixed(3)}, denom=${speedForFraction})`);
                } catch (e) {}
              }
            }
            // Compute global time_after_winner (rider.prel_time - minPrel across all riders)
            ...
            // update latestPrelTime to reflect assigned times
            try { setLatestPrelTime(prev => Math.max(prev || 0, latestPtLocal)); } catch (e) {}
